...
class BestFirstSearchSolver(AbstractSearchSolver):
    ...
    def searchStep(self):
        ...
        if self.fringe.isEmpty():
            return (False, False, "Fail")
        nextState, nextCost = self.fringe.delete()
        ...
        for n in neighbors:
            ...
            if (not visitedMatch) and (not fringeMatch):
                ...
                if verbose:
                    print("    Neighbor was already in fringe, comparing neighbor's cost", n)
                tempList = []
                tempNum = 0
                while (n != self.fringe.firstElement()[0]):
                    deletedFringe = self.fringe.delete()
                    tempList.append(deletedFringe)
                    tempNum += 1
                if verbose:
                    print("      with fringed cost", self.fringe.firstElement()[0])
                if self.fringe.firstElement()[1] > n.getCost():
                    if verbose:
                        print("        Replacing fringe with neighbor")
                    self.fringe.removeValue(n)
                    self.fringe.insert(n, n.getCost())
                for i in range(tempNum):
                    self.fringe.insert(tempList[i][0], tempList[i][1])
            elif visitedMatch:
                if verbose:
